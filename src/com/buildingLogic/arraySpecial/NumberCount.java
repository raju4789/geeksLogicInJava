...
public class NumberCount {
	...
	public static void main(String[] args) {
		...
		calculateNumberCountThree(inputArr);
...
occurrences.
	...
	//Chintu's approach
	static void calculateNumberCountThree(int[] inputArr){
		int inputArrLength=inputArr.length;
		int[] tempArr=new int[inputArrLength];
		int tempIndex=0;
		for (int i = 0; i < inputArrLength ; ++i) {
			int count=1;
			tempArr[tempIndex]=inputArr[i];
			if(!alreadyExists(inputArr[i], tempArr, tempIndex)){
				for (int j = 0; j < inputArrLength ; ++j) {
					if((i!=j) && inputArr[i] == inputArr[j]){
						++count;
					}
				}
				System.out.println(inputArr[i]+" repeated "+count+" times");
			}
			++tempIndex;
		}
	}
